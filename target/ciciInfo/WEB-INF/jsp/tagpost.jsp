        ...
        <div class="content">
            <div class="pagetitle">
                <c:choose>
                    ...
                    <c:when test="${empty term}">
                        <input type="hidden" id="tagid" value="-1">
                        <h1><i class="i itag tag-channel"></i>空空如也</h1>
                    </c:when>
                    <c:otherwise>
                        <input type="hidden" id="tagid" value="${term.termId}">
                        <h1><i class="i itag tag-channel"></i>${term.name}</h1>
                    </c:otherwise>
                </c:choose>
            </div>
            <%--正文--%>
            ...
            <script>
                ...
                function createTable(currPage, limit) {
                    if($("#tagid").val() == -1) return;
                    $.ajax({
                        url:'/tagsposts',
                        method:'post',
                        async: false,
                        data:{'tid': $("#tagid").val(), 'start': currPage, 'size':limit},
                        beforeSend: function () {
                        ...
                        success: function(data){
                            ...
                            $("#cont").append(msg);
                            history.pushState(null, null, "tags?tid="+$("#tagid").val()+"&page="+currPage);
